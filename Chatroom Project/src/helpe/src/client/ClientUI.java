...
public class ClientUI extends JFrame implements Event {
	...
	JButton exportChatButton = new JButton("Export chat");
    ...
    void exportChat() {
    	try {
    		...
    		if (file.createNewFile()) {
    			System.out.println("Chat History file has been created!");
    		}
    		else {
    			System.out.print("Chat History file already exists, goofball");
    	...
    	for (Component c : components) {
    		...
    		if (j != null) {
    			stringbuilder.append(j.getText() + System.lineSeparator());
    		}
    	}
    	
    	try {
    		FileWriter filewrite = new FileWriter("ChatHistory.txt");
	...
	User u = new User(name, "<font color=purple>%s</font>");
    ...
    public void onIsMuted(String clientName, boolean isMuted) {
    	...
    	while (iter.hasNext()) {
    		...
    		if (u.getName().equalsIgnoreCase(clientName)) {
    			if (isMuted) {
    				u.setName(clientName, "<font style:red>%s</font>");
    			}
    			else {
